...
func BeforeApp(c *cobra.Command) error {
	if verbose, _ := c.InheritedFlags().GetBool("verbose"); verbose {
...
func ProjectBuild(p *project.Project, c *cobra.Command) {
	logrus.Info("ProjectBuild executing...")
	fmt.Println("ProjectBuild executing..")
	logrus.Info("Args: ",c.Flags().Args())
